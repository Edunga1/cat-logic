# Summary

## 🐱 Logic

보고 들은 것에 대한 생각, 용어 정의 등을 자유롭게 씁니다.

* [객체 지향 프로그래밍 (OOP)](cat-logic/e99ecf640015fd820093db0aaaaec50f.md)
* [Google Analytics 수동 테스트하는데 활성화 사용자가 0으로 뜨는 현상. 트래픽 쫓아가기.](cat-logic/eeccbf9da173e02874538756fc0c4909.md)
* [[Linux] sed - 파일 특정 라인만 읽기](cat-logic/c07d9e8105ed9102e926a0542ce9c880.md)
* [JSDoc 파라미터 상수 정의하기 + VSCode Rename Symbol!](cat-logic/e148c978eb45347beb4f2cf9a971cf81.md)
* [NodeJS 서버 로컬 요청만 허용하기](cat-logic/5a662df6f9cfe4dcf1ebb542a2def358.md)
* [pm2 deploy 시 주의할 점](cat-logic/nodejs-pm2-deploy-note.md)
* [안정된 의존관계 원칙(Stable Dependencies Principle)](cat-logic/stable-dependencies-principle.md)
* [The Twelve-Factor App](cat-logic/the-twelve-factor-app.md)
* [TED - 리누스 토발스(Linus Torvalds): 리눅스의 기본 철학](cat-logic/ted-linustorvalds.md)
* [개발에 대한 사소한 생각 - 트렌드](cat-logic/minor-thinking-about-development.md)
* [초콜릿 문제](cat-logic/chocolate-problem.md)
* [용어 - 테스트 관련](cat-logic/jargon-pertinent-to-test.md)
* [용어 - bash**rc** rc?](cat-logic/jargon-rc.md)
* [용어 - Business Rule, Business Logic, Logic](cat-logic/jargon-business-logic-rule.md)

## Offline

* [구글캠퍼스 - 구글 플레이에서 꼭 지켜야 할 두 가지](offline/google-campus-two-things-you-must-keep-in-google-play.md)
* [GDG WebTech Workshop: 측정하는 놈, 로딩하는 놈, 그리는 놈](offline/gdg-webtech-workshop-nnn.md)

## methodology

* [테스트 도구](methodology/tools-for-tdd.md)
* [테스트 주도 개발 (**T**est **D**riven **D**evelopment)](methodology/test-driven-development.md)

## OOP

* [다형성 - upcasting & downcasting](oop-paradigm/polymorphism-up-down-casting.md)

## Design Pattern

* [SOLI**D** - Dependency Inversion](pattern/dependency-inversion.md)
* [Design Pattern](pattern/design-pattern.md)
* [Iterator Pattern](pattern/iterator-pattern.md)
* [Factory Pattern](pattern/factory-pattern.md)
* [MVC Pattern](pattern/mvc-pattern.md)

## Algorithm Practice

* [땅따먹기 게임](algorithm-practice/hopscotch.md)

## Shell

* [Command - scp](shell/scp-secure-copy.md)
* [Command - set -x](shell/set-option-ex.md)
* [Command - curl - downloaded file is too small.](shell/curl-downloaded-file-but-zero-size.md)

## CSS

* [Grid Layout](css/grid-layout.md)

## Javascript

* [자바스크립트 기본](javascript/javascript-basic.md)
* [string.search 사용 시 예외](javascript/string.search-exception.md)
* [foo = (bar = 1, bar + 1)](javascript/two-operations-at-once.md)
* [Javascript: The Good Parts - 자바스크립트에서 권장하지 않는 것](javascript/javascript-good-parts-reconsidered.md)

## AngularJS

* [Johnpapa's AngularJS Style Guide - Modularity](javascript-angularjs/angularjs-style-modularity.md)
* [Johnpapa's AngularJS Style Guide - Component](javascript-angularjs/angularjs-style-component.md)
* [AngularJS 테스트하기](javascript-angularjs/angularjs-testing.md)
* [VSCode - AngualrJS에서 ViewModel 정적 타이핑하기](javascript-angularjs/static-typing.md)
* [Migrating 1.5 to 1.6](javascript-angularjs/migration-1.5-to-1.6.md)

## NodeJS

* [상호 참조 시 문제](nodejs/cross-require.md)
* [Typescript - 환경 구성 하기](nodejs/nodejs-with-typescript.md)
* [Testing - 테스트 도구](nodejs/test-tools.md)
* [Testing - Mocha 사용 시 require.main.require 이슈](nodejs/mocha-require.main.require-issue.md)
* [Testing - SinonJS Date Mocking 하기](nodejs/sinon-js.md)
* [Testing - proxyquire - vs. rewire](nodejs/proxyquire-vs-rewire.md)
* [Testing - proxyquire - 모듈 로드 순서 문제](nodejs/proxyquire-module-import-order.md)
* [VSCode - 타입 힌팅 사용하기 (blog)](nodejs/type-hinting-in-vscode.md)

## Database

* [정규화](database/database-normalization.md)
* [Style - Table Naming Convention - 단수?복수? camelCase?snake_case?](database/table-naming-convention.md)

## Data Structure

* [Associative Array와 Hash Table](data-structure/associative-array-and-hash-table.md)
* [Graph](data-structure/graph.md)
* [Heap](data-structure/heap.md)
* [Set(집합)](data-structure/set-abstract-data-type.md)

## Docker

* [OSX 환경 구성](docker/docker-environment-setup-osx.md)
* [OSX 환경 구성 - docker for mac](docker/simply-docker-environment-setup-osx.md)
* [Dockerfile](docker/dockerfile.md)
* [Volume](docker/dockerfile-volume.md)
* [Issue - 빌드 시 유의사항](docker/docker-issue-build.md)

## Git

* [3rd - cherry-pick](git/git-cherry-pick.md)
* [Git 명령어 자동 완성 사용하기](git/git-command-completion.md)
* [Git ssh 사용하여 자동 로그인 사용하기](git/git-ssh.md)

## Gitbook

* [이슈](gitbook/gitbook.md)

## Github

* [API - Release 된 파일 다운받기](github/github-api-downloading-release.md)

## PHP

* [OSX - VSCode로 PHP 디버깅 하기](php/debugging-through-vscode.md)

## Ionic

* [crosswalk-webview 플러그인 설치 시 주의사항](ionic/caution-to-installing-crosswalk-webview-plugin.md)
* [Error - Could not find gradle wrapper within Android SDK](ionic/error-could-not-find-gradle-wrapper-within-android-sdk.md)

## Unity

* [Unity Player](unity/unity.md)
* [OSX - 커맨드라인으로 유니티 빌드하기](unity/building-with-unity-through-command-line-osx.md)
* [MonoBehaviour](unity/mono-behaviour.md)
* [Issue - Sprite Packer가 동작하지 않는 현상](unity/sprite-packer-does-not-work.md)

## Tools

* [Postman - API Management](tools/postman.md)
