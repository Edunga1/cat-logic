...
주소창 등에서 fuzzy search가 되므로 `#태그1 #태그2`로 어느정도 검색된다.

![chrome bookmarks tagging](./res/chrome-bookmarks-tagging.png)

이미지와 같이 북마크 이름 끝에 태그를 붙이고, 태그 순서를 바꿔도 검색이 된다.
단점은 검색어에 부분 일치하는 엉뚱한 북마크가 검색되는 점이다.

결국 크롬을 사용하다보면 북마크를 잘 사용하지 않게 된다.
기억할 것이 있으면, 위키에 정리한다. 북마크 바 정도만 빠른 이동을 위해 주로 사용한다.
이전의 카테고리를 만드는 등 정리하는 것은 웹 페이지가 사라지는 경우도 많아서 하지 않는다.
