...
작업자가 휴무일에 작업하고 리뷰 요청을 하는 경우가 있다.
시간이 남아서 처리했고, 반영은 업무일에 하겠다는 의도다.
**하지만 이 경우에는 Draft 상태로 PR을 올리는게 좋겠다.**
리뷰어 입장에서는 작업의 중요도를 알 수 없고, 피드백을 줬을 때 기본적으로 답변이 오는 것을 기대하기 때문이다.

Bitbucket의 경우 Draft 상태로 올리면 리뷰어에게 알림이 가지 않는다.

작업자가 오늘 처리하지 않는 것을 정확히 안다면 리뷰어는 우선 순위를 조절할 수 있다.
