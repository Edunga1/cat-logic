...
### 터미널에서 한글을 두 번 입력해야 하는 문제

터미널 앱에서 한글을 입력할 때, 첫 번째 엔터는 조합 완료를 위해서, 두 번째 엔터는 입력 완료. 총 두 번 엔터를 눌러야 반영하는 문제.

![iTerm2 korean enter](./res/macos-iterm2-korean-enter.gif)

이미지와 같이 한글 입력시 밑줄 표시가 나타나고,
첫 번째 엔터에서 밑줄이 사라지면서 조합이 완료되고,
다시 한 번 엔터를 눌러야 명령을 실행한다.
조합 완성은 엔터 대신 방향키, 한영전환 등으로 대신할 수는 있다.

영어 입력만 한다면 이런 불편함은 없다.
테스트한 결과 기본 터미널 앱을 포함하여 iTerm2 등 다른 터미널 앱에서도 동일한 문제가 발생한다.
...
| kitty        | X                                |
| Ghostty      | X                                |
| Alacritty    | O, 특수문자가 조합 완료로 사용됨 |
| WezTerm      | O                                |

Alacritty만 한글 입력 시 엔터 한 번으로 명령이 실행된다.
하지만 `?/!,.` 등 특수문자가 조합 완성으로 인식되어서  특수문자를 두 번 입력해야 하는 문제가 있다.
구름 입력기를 사용하면 특수문자 문제는 해결되지만, 엔터 두 번 입력 문제가 발생한다.

WezTerm은 특수문자, 엔터 문제 모두 해결된다.
1시간도 사용하지 않았지만, 한글 처리가 매끄럽다.
