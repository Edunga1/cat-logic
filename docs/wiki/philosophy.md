...
아무리봐도 모르겠지만, 다른 분야의 이야기를 보려고 하는 거다.
...
그래도 키워드는 주워 듣고, 관련 뉴스를 보면 조금 감이 잡힌다.

[GeekNews](https://news.hada.io/)는 서비스 시작 당시부터 보고있는 뉴스 사이트이다.\
주제는 IT와 사업 등 광범위하다. [Hacker News](https://news.ycombinator.com/)가 모티브인데, 그래서 사업과 개발에 대한 뉴스가 많다.
예전에는 내 개인 사업을 하는 것에 대해서 꿈도 꾼 적이 없지만, 이제는 시도는 해보지 않을까 싶다.
...
정말로 내 생각도 조금씩 변경되는 것을 인지하게 된다.
그래서 얻은 교훈은, 앞으로 건강한 컨텐츠에 노출되도록 신경 써야겠다는 것. 그리고 계속해서 모르는 것을 찾아보자는 것이다.
