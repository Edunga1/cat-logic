...
건강한 콘텐츠에 노출되도록 신경 써야겠다. 그리고 계속해서 모르는 것을 찾아보자.
...
프로그래밍에서 버퍼는 데이터를 임시적으로 저장하는 공간이다.

임시란 개념은 개발자에게 안심감을 준다.\
예를들어, 셸에서 `cat foo.txt | awk '{print $1}' | sort -u | rg 'pattern'`와 같은 명령어 조합을 한 번에 도출해내기 어렵지만,
각 단계마다 결과를 확인하면서 임시 결과를 확인하면 수월하게 도달할 수 있다.

[Factorio](/docs/wiki/game.md#factorio)는 이 버퍼 개념을 잘 녹여낸 게임이다.\
생산 라인을 구축하는 게임으로써 한 번에 많은 퍼즐에 맞닥뜨린다.
한 번에 생산 건물간 연동을 시도하면 공간이 좁아지고 구조가 복잡해지기 마련이다.
하지만 버퍼인 플레이어가 수동으로 넣어준다면, 우선 전체 시스템이 동작하게 만들고, 적은 공간으로 구조를 단순화할 수 있다.
어려운 것은 버퍼로 해결하고, 쉬운 것부터 시작하고 버퍼로 동작하는 곳을 점점 줄여나가는 것이다.
그래서 팩토리오는 복잡하지만, 언제든 시도할 수 있다는 안심감에 포기하지 않고 계속 시도하게 만들도록 설계가 잘 되어 있는 게임이다.
