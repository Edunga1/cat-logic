...
예를 들어 Pylint는 [broad-exception-caught](https://pylint.readthedocs.io/en/latest/user_guide/messages/warning/broad-exception-caught.html)와 [consider-using-f-string](https://pylint.readthedocs.io/en/latest/user_guide/messages/convention/consider-using-f-string.html)을 잡아주지만 Ruff는 그렇지 않다.
...
- `uv init`: uv 프로젝트 초기화. `pyproject.toml` 파일이 생성된다. `main.py`와 `.python-version` 파일도 생성하는데 필수적인 파일은 아니다.
...
다른 도구처럼 `uv shell` 같은 명령어를 제공하지 않는 것과 명령어를 모두 일일이 타이핑해야 하는 것은 불편한데,
...
처음 uv를 이용할 때 난감한 점은, `python` 명령어를 대신해서 `uv`를 사용해야 한다는 것이다.
`pip`도 마찬가지. `python app.py` 대신 `uv run python app.py`를 사용해야 하며, `pip install PACKAGE_NAME` 대신 `uv add PACKAGE_NAME`을 사용해야 한다.
`pip list` 대신 `uv tree`를 사용한다. REPL 또한 `uv run python`으로 실행한다. python 명령어로 시작하면 패키지를 찾지 못한다.
