...
Git으로 파일 수정 내역을 추적할 수 있지만, 파일 이동에 취약하다.

front matter 처럼 문서 외 정보가 없으면 사이트를 만들 때 불편한 점이 몇 개 있다.
문서 제목으로 무엇을 채택할 지, 요약 정보를 보여준다면 어떻게 요약해야 할 지.
front matter로 표현하면 해당 필드를 추가하면 간단하다.
지금은 첫 번째 헤더를 문서 제목으로, 첫 번째 헤더의 내용을 요약 정보로 사용하고 있다.

정적 사이트를 위한 프론트 작업을 하거나 텍스트 임베딩를 사용해보는 등 실험적인 프로젝트를 겸하고 있다.
...
### 2023-11-10 임베딩을 이용한 관련 문서 기능 추가

이전에는 문서 내용의 내부 링크를 찾아서 추천해주는 것이었다.
...
### 2023-11-15 한국어 slug 지원

나무위키처럼 한글과 depth path로 퍼마링크의 구조적이고, 의미 있는 URL을 만들고 싶었다.

작업 커밋: https://github.com/Edunga1/cat-logic/commit/b8b32647aaf301e38acb61cfaabba9cc906f8bbe

### 2024-03-26 커밋 날짜를 기준으로 문서 정렬
