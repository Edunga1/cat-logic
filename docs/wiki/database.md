...
Shopify가 재고 관리를 Redis에서 MySQL로 교체하여 트랜잭션 원자성을 보장한 경험을 공유한 글.

- https://news.hada.io/topic?id=30006
- https://shopify.engineering/scaling-inventory-reservations

이커머스의 재고 관리는 과잉 판매되어 주문을 취소하거나 반대로 재고가 남아있는데도 판매하지 못하는 상황을 방지하기 위해 일반적으로 Redis를 사용하여 순차 처리하여 재고 문제를 해결하기 마련이다.

Shopify도 마찬가지였다고 한다. 각 항목별 수량 키를 가지고, 예약하면 수량을 차감,  취소하면 수량을 증가 처리했다.
문제는 Redis는 별도 시스템이다 보니 클레임 단계의 결제 처리(MySQL)과 재고 차감(Redis)간 두 연산간 원자성을 보장할 수 없는 문제가 있었다.
두 번째 문제는 Redis의 multi-location를 고려하지 않아서 클러스터를 도입하기에는 추가적인 운영 부담이었다고 한다.

Shopify는 이를 해결하기 위해 열을 재고로 표현하는 대신, 재고 풀을 별도 테이블에 행으로 표현했다.
10개의 재고가 있는 경우 풀에 10개의 행을 미리 생성해둔다.
예약이 발생하면 `SELECT ... FOR UPDATE SKIP LOCKED` 쿼리를 사용하여 행을 잠그고,
판매를 위해 예약 테이블에 데이터를 생성하고 잠근 재고 풀의 행은 삭제하여 재고를 차감하는 흐름이다.
이를 통해 다른 트랜잭션을 통해 동시에 예약이 발생하더라도 SKIP LOCKED로 인해 잠긴 행은 건너뛰고 사용 가능한 다른 행을 가져오므로 동시 처리가 가능하다.

하지만 재고 수량을 수십만개 이상으로 미리 생성해두면 스캔 속도가 느려진다.
이 문제를 해결하기 위해서 재고 풀을 1,000개로 제한하고, 소진되면 재고 보충 프로세스를 통해 재고를 다시 보충한다. 1,000개는 검색 속도와 운영 모니터링을 통해 조정한 값이라고 한다.

`SELECT ... FOR UPDATE SKIP LOCKED` 쿼리는 MySQL 기본 격리 수준인 REPEATABLE READ에서 gap lock을 발생한다.
새로운 행을 삽입할 수 없기 때문에 READ COMMITTED로 격리 수준을 변경해야 한다.
