...
예를 들어 `iron reflexes(10661)`의 경우 `dexterity(8544)`로만 연결되어 있어서 `"out"=["8544"]`로 설정되어 있고,
...
예를 들어 1:1 전투나 1:N 전투에 따라서 DPS가 달라진다고 할 수 있는데, 이런 상황을 시뮬레이션할 수 있는 설정 기능을 제공한다.
...
예를 들어 지역에서 유닛을 1기씩 제거를 반복하는 액션을 만들었을 때,
...
팩토리오는 C++로 개발된 공장 테마의 시뮬레이션 게임이다.
MacOS, Windows, Linux에서 플레이할 수 있다.

2026년 1월에 시작했고, 아직 플레이타임은 적지만 갓겜.
자동화와 효율 최적화 그리고 적과의 전투가 주요 게임 플레이 요소다.

할인을 하지 않는 정책으로 유명하다. 출시 이후로 단 한 번도 할인을 하지 않았고, 가격이 오르기만 했다.

[결정론적인 멀티스레딩은 어렵습니다](https://gall.dcinside.com/factorio/66131) 글에서 멀티플레이와 멀티스레딩에 대한 고충을 엿볼 수 있다.\
상세하게 개발 과정을 공유해서 흥미롭고, 읽어보면 진솔하게 작성한 것을 볼 수 있다:
...
플레이어가 건설하는 수많은 공장, 벨트 위로 이동하는 수많은 아이템, 기지를 떠다니는 수 천대의 로봇이 동시에 존재하면서도 쾌적하게 플레이할 수 있도록
[커스텀 엔진](https://www.factorio.com/support/press-kit)으로 개발되었다.
C++로 개발했고, 레벨 생성과 모드는 Lua 스크립트를 지원한다.

[알레그로로 개발을 시작했지만, 안정성을 위해 그래픽 엔진을 재작성 했다고 한다](https://www.factorio.com/blog/post/fff-230).
[알레그로](https://liballeg.org/)는 게임 개발을 위한 라이브러리라고 하는데, 맞는지는 모르겠다.
[SDL](https://www.libsdl.org/index.php)로 창 관리와 이벤트, 입력 처리를 한다고 한다.

알레그로를 제거하고 그래픽 엔진을 재작성하는 작업은 고통스럽지만 매우 능숙한 일이라 문제는 없었다고.

## Hearhstone

블리자드의 카드 게임.
메인 게임 모드 만큼이나 전장 모드가 인기있다.
오토 배틀러 모드로 전장 구축만 하면 자동으로 전투가 진행된다.

### hsreplay

hsreplay는 전장 모드의 통계 데이터로 승률을 제공하는 서드파티 도구다.

[Bob's buddy 소개](https://articles.hsreplay.net/2020/04/24/introducing-bobs-buddy/)라는 글에서는
전장 모드의 실시간 승률을 어떻게 계산하는지 알려준다. 그 방법은 단순히 시뮬레이션하는 것. 최대 10,000번의 전투를 통해서 매 턴 승률을 계산한다고.

실제로 카드가 대거 변경되는 시즌 초반에는 승률이 정확하지 않은 것이 구현 방식을 반증한다.
