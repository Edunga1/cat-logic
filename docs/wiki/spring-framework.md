...
logback.xml의 수정이 되는 것은 `fswatch`로 확인했다:
...
reconfigure가 되는지 확인하기 위해선 logback의 로그를 확인해야 하는데,
`<configuration debug="true">` 디버그 설정을 추가하면 된다.

설정 파일 변경으로 인해 logback이 재설정되는 것을 확인했다:
...
Spring 확장 설정을 읽지 못해서 무시된다는 로그도 확인했다:
