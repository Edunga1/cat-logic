...
국내법에서 고층 건물을 120m로 정의하니, 스타쉽은 고층 건물을 우주로 보내는 것과 같다.

총 2단으로 구성되어 있다.
2단부 "Starship"은 우주선 자체, 1단부 "Super Heavy"는 Booster로 궤도에 올리기 위한 부분이다.
Super Heavy는 궤도에 도달하기 전에 분리되어 다시 지구로 돌아온다. 다시 회수하여 재사용하는 것이 목표다.
...
2024년 3월 14일 오후 9시경. 스타쉽 3차 발사가 이루어졌다.
...
33개의 랩터 엔진은 Starship을 우주로 올려놓기 위한 추력을 제공한다.\
...
- 분리된 Super Heavy는 정상적으로 바다에 착수했다.
...
착수하는 Super Heavy의 모습. 3개의 엔진을 점화하여 Landing Burn을 통해 역 추력을 사용한다.
...
이번 시험에서 성공한 임무는 1단 Super Heavy를 발사탑의 집게팔인 Chopsticks에 착륙시킨 것이다.
...
이는 [트위터 글](https://x.com/SpaceX/status/1858241453824287192)에서 고화질로 확인할 수 있다.
...
결과는 Super Heavy 부스터를 Chopsticks에 성공적으로 착륙시켰지만...\
...
한국 시간으로 2025년 8월 27일 오전 8시 30분에 10차 시험이 진행되었다.
이번 시험의 주 목적은 스타쉽의 내구성 테스트로 보인다.

안될과학에서 [10차 시험을 해설](https://www.youtube.com/watch?v=M5eTH_NVlHQ)했다.
아래 정리는 해설 영상을 참고했다.

- 2026년에 화성에 스타쉽을 발사하는 것이 스페이스X의 목표.
- 런치 패드를 여러 개 건설하고 있는데, 1단과 2단을 함께 회수하기 위함이라고.
- 1단부의 가장 중심부 엔진 3개는 지면 착륙 시에 사용하고 나머지는 모두 끄게 되는데,
  이번에는 응급 상황 실험을 위해 3개의 엔진 중 1개를 끈 상태로 호버하여 바다에 착수하는 시험을 진행했다.
  따라서 Chopsticks에 착륙하지는 않았다. 바다에는 성공적으로 착수했다.
- 이번 시험에서는 스타링크 모형을 싣고 궤도에서 사출하는 것을 시험했고, 성공했다.
- 나중에는 실제 스타링크의 위성을 스타쉽으로 설치하는 것이 목표라고 한다.
- 우주에서 엔진을 끄고 재점화하는 테스트를 성공적으로 수행했다.
- 2단부 동체에 스트레스를 줘서 얼마만큼 버티는지 시험했다. 날개 일부가 떨어져 나갔지만 버텨냈다.
- 2단부의 바다 착수를 성공했다.
...
- 1단(first stage): 3개의 Falcon 9 로켓을 묶은 형태로, 2개의 사이드 부스터와 1개의 코어 부스터로 구성된다.
...
2026년 4월 29일 한국 시간 22시경에 VIASAT-3 F3 위성을 발사했다.

이미지는 안될과학 생중계 영상에서 캡처했다.

![VIASAT-3 F3 launching](./res/falcon-heavy-viasat-3-f3-launching.png)

오른쪽 하단에 3개의 부스터를 나타내는 텔레메트리 그래픽이 보인다.
총 27개의 엔진을 나타내는 동그라미에 불이 들어온 것을 볼 수 있다.

![VIASAT-3 F3 separating](./res/falcon-heavy-viasat-3-f3-separating.png)

사이드 부스터 분리를 위해 엔진을 끄는 모습.
코어 부스터는 당분간 유지하는 모양이다.

![VIASAT-3 F3 separated](./res/falcon-heavy-viasat-3-f3-separated.png)

1단이 분리되어, 2단 엔진의 텔레메트리로 교체된 모습.
이제 궤도로 진입한다.

![VIASAT-3 F3 side-booster landing](./res/falcon-heavy-viasat-3-f3-booster-landing.png)

사이드 부스터를 회수하기 위해 착륙하는 모습!
왼쪽 사이드 부스터는 성공적으로 착륙, 오른쪽 사이드 부스터는 가운데 엔진만 점화된 채 착륙 중이다.
...
2025년 1월 13일 오전 6시(UTC). 첫 시험이 예정되었다. 미션 이름은 NG-1.
