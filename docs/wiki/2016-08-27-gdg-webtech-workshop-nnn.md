...
- [개요](#개요)
- [Slides](#slides)
- [프론트엔드 개발자를 위한 크롬 렌더링 성능 인자 이해하기](#프론트엔드-개발자를-위한-크롬-렌더링-성능-인자-이해하기)
  - [하드웨어 가속](#하드웨어-가속)
  - [CPU와 GPU 사이에 존재하는 이슈](#cpu와-gpu-사이에-존재하는-이슈)
    - [1. 서로 다른 메모리 공간](#1-서로-다른-메모리-공간)
    - [2. 메모리 한계](#2-메모리-한계)
    - [3. 데이터가 자주 변경된다.](#3-데이터가-자주-변경된다)
  - [GPU](#gpu)
    - [GPU가 잘하는 것](#gpu가-잘하는-것)
    - [GPU의 약점](#gpu의-약점)
      - [하지만 더 큰 이슈는 **CPU의 처리 시간**](#하지만-더-큰-이슈는-cpu의-처리-시간)
  - [렌더링 성능의 주요 인자](#렌더링-성능의-주요-인자)
- [크롬의 하드웨어 가속 렌더링 메커니즘](#크롬의-하드웨어-가속-렌더링-메커니즘)
  - [웹페이지의 렌더링](#웹페이지의-렌더링)
  - [레이어 모델](#레이어-모델)
  ...
  - [렌더링 이슈](#렌더링-이슈)
    - [Reflow](#reflow)
    - [Repaint](#repaint)
    - [Reflow / Repaint 발생 요인](#reflow-repaint-발생-요인)
  - [크롬 개발자도구](#크롬-개발자도구)
    - [Timeline](#timeline)
  ...
  - [크롬에서의 전반적인 렌더링 흐름](#크롬에서의-전반적인-렌더링-흐름)
- [렌더링 최적화 방법](#렌더링-최적화-방법)
  - [가장 간단한 방법 : 최소한으로 레이어 분리](#가장-간단한-방법-최소한으로-레이어-분리)
    - [translateZ(0)](#translatez0)
  - [강제적인 레이어 분리가 만능이 아닌 이유](#강제적인-레이어-분리가-만능이-아닌-이유)
    - [레이어 분리는 텍스쳐 이미지의 분리를 의미한다.](#레이어-분리는-텍스쳐-이미지의-분리를-의미한다)
  - [CSS: will-change](#css-will-change)
- [Instant and offline apps](#instant-and-offline-apps)
  - [Instant Loading](#instant-loading)
    - [압축](#압축)
    - [작은 이미지 사용](#작은-이미지-사용)
    - [Round Trips 줄이기](#round-trips-줄이기)
...
- [Service Worker](#service-worker)
  ...
  - [Service Worker](#service-worker)
    - [기능: 오프라인 캐쉬](#기능-오프라인-캐쉬)
    - [기능: 백그라운드 동기화](#기능-백그라운드-동기화)
    - [규칙, 장점](#규칙-장점)
<!--toc:end-->
