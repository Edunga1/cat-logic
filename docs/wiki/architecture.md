...
프로그래밍에서 직교성은 단순함과 유연함을 가리킨다.
복잡성을 배제하고 각 기능이 독립적으로 조정될 수 있도록 만든다.

높은 직교성은 [게임](/docs/wiki/game.md)에서의 창발적 재미에 비유할 수 있다.
독립적인 요소들이 만들어내는 예측하지 못한 상호작용이 매력이다

직교성은 팀 구조, 테스팅, 소프트웨어 설계 등 다양한 곳에서 언급된다.
...
[GitHub](/docs/wiki/github.md)의 개발 문서는 GraphQL과 REST API 모두 지원하고,
다양한 케이스가 있어서 인터페이스 설계에 참고할만하다.
...
저장소에 [별 주기 API](https://docs.github.com/en/rest/activity/starring)와 같이 toggle 방식은 PUT/DELETE 두 개의 API로 제공한다.
...
토글 형태의 다른 API도 참고해보면, 리소스 주체에 따라서 API path가 어디서 구분되는지 달라진다.

| API            | PUT                                    | DELETE                                    |
|----------------|----------------------------------------|-------------------------------------------|
| Starring       | PUT /user/starred/{owner}/{repo}       | DELETE /user/starred/{owner}/{repo}       |
| Watching       | PUT /repos/{owner}/{repo}/subscription | DELETE /repos/{owner}/{repo}/subscription |
| Starring(Gist) | PUT /gists/{gist_id}/star              | DELETE /gists/{gist_id}/star              |

모두 사용자 단위 리소스지만, 사용자에 둘 것인지 저장소, Gist에 둘 것인지 결정에 따라 API path가 달라진다.
Starring과 Watching은 각각 리소스와 저장소 단위로 구분하다보니 비슷한 기능임에도 path 구조가 달라진다.
