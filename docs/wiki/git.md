...
여러 개의 커밋을 묶는 데, `git reset HEAD~#` 후 다시 커밋을 생성하는 방법도 있지만,
여러 개의 커밋을 남겼을 경우, 메시지들이 사라진다는 단점이 있다.
...
이진 탐색을 이용하여 버그가 처음 발생한 커밋을 찾는데 사용한다.
메뉴얼 상으로는 버그라고 하지만, 특정 컨텐츠를 가진 커밋을 찾아내는 것이 목적으로 사용할 수 있다.
...
4. *자동으로 중간 커밋으로 체크아웃 됨. 버그를 확인한다.*
5. `git bisect good` or `git bisect bad` 명령어로 4-5 반복.
6. *버그가 처음 발생한 커밋에서 자동 종료.*

*bisect*는 둘로 나눈다는 뜻이다.

이진 검색을 사용하기 때문에, 아무리 커밋이 많아도 `log(n)` 안에 찾을 수 있다.
커밋이 1억 개가 있더라도 27번만 검색하면 된다.

올바른 커밋과 잘못된 커밋을 먼저 선정하면, git이 자동으로 중간 커밋으로 체크아웃하는 흐름이다.\
사용자는 현재 커밋에서 동작을 확인하고 올바른지 잘못되었는지 알려주고 git이 다음 중간 커밋으로 체크아웃하는 것을 반복한다.
